//! GTC instance.

use super::register::{CntFreq, RegisterBlock};
use crate::cmu;
use core::marker::PhantomData;

/// GTC.
pub struct Gtc {
    reg: *const RegisterBlock,
    _private: PhantomData<()>,
}

impl Gtc {
    /// Create a new GTC instance.
    pub const fn __new(reg: *const RegisterBlock) -> Self {
        Self {
            reg,
            _private: PhantomData,
        }
    }

    /// Get a reference to the register block.
    pub const fn reg_ref(&self) -> &RegisterBlock {
        unsafe { &*self.reg }
    }
}

impl Gtc {
    /// Initialize the GTC instance.
    pub fn init(self, freq: CntFreq, cmu: &cmu::RegisterBlock) -> Self {
        let reg = self.reg_ref();
        let clk = &cmu.clock_gtc;
        unsafe {
            clk.modify(|v| v.enable_bus_clk());
            clk.modify(|v| v.enable_module_reset());
            riscv::asm::delay(500);
            clk.modify(|v| v.disable_module_reset());
            reg.config.modify(|cfg| cfg.set_fdiv(6));
            reg.cnt_ctrl
                .modify(|ctrl| ctrl.disable_cnt_on_dbg().set_freq(freq).enable_cnt());
        }
        self
    }

    /// Get current tick.
    pub fn get_tick(&self) -> u64 {
        let reg = self.reg_ref();
        let low = reg.cnt_value_low.read() as u64;
        let high = ((reg.cnt_value_high.read().cnt_value_high() & 0xFFFFF) as u64) << 32;
        high | low
    }

    /// Delay us.
    pub fn delay_us(&self, us: u64) {
        let reg = self.reg_ref();
        let one_sec_tick: u64 = match reg.cnt_status.read().fcack() {
            CntFreq::Freq4M => 4_000_000,
            CntFreq::Freq1M => 1_000_000,
            CntFreq::Freq250k => 250_000,
        };

        let start_tick = self.get_tick();
        let target_ticks = us * one_sec_tick / 1_000_000;

        loop {
            let elapsed_tick = self.get_tick().wrapping_sub(start_tick);
            if elapsed_tick >= target_ticks {
                break;
            }
        }
    }

    /// Delay ms.
    pub fn delay_ms(&self, ms: u64) {
        self.delay_us(ms * 1_000);
    }
}
